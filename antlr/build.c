...
Graph buildAction( char *action, int file, int line, int is_predicate )
{
  ...
  if (is_predicate) {
    ...
    *t='\0';
    ...
    char  *strStart=a->action;
    char  *strEnd;
    strEnd=strStart+strlen(strStart)-1;
    for ( ; strEnd >= strStart &&  isspace(*strEnd); strEnd--) *strEnd=0;
    while (*strStart != '\0' && isspace(*strStart)) strStart++;
    if (ci_strequ(strStart,"nohoist")) {
      a->noHoist=1;
    }
  }
